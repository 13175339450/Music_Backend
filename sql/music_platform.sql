...
-- 4. [新增] 关联数据回填
-- =========================================================
-- 补充说明：上面的步骤创建了 musician 记录，但 user 表里的 musician_id 字段还是空的。
-- 必须执行这一步，否则查询用户详情时无法关联到音乐人信息。

UPDATE `user` u
    INNER JOIN `musician` m ON u.id = m.user_id
    SET u.musician_id = m.id
WHERE u.is_musician = 1
  AND u.musician_id IS NULL;